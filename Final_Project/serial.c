...
//    SP4BRGL = 103; 			//set baud rate to 103 = 9600bps
    SP4BRGL = 52; 			//set baud rate to 52 = 19200bps
    SP4BRGH = 0;			//not used
...
char getCharFromRxBuf(void){
    if (RxBufReadCnt>=RX_BUF_SIZE) {RxBufReadCnt=0;} // if buffer full reset read count to zero
    return EUSART4RXbuf[RxBufReadCnt++];             // return next byte
}

// add a byte to the buffer
void putCharToRxBuf(char byte){
    if (RxBufWriteCnt>=RX_BUF_SIZE) {RxBufWriteCnt=0;}  // if buffer full reset write count to zero
    EUSART4RXbuf[RxBufWriteCnt++]=byte;                 // write byte in next spot
}
...
char getCharFromTxBuf(void){
    if (TxBufReadCnt>=TX_BUF_SIZE) {TxBufReadCnt=0;} // if buffer full reset read count to zero
    return EUSART4TXbuf[TxBufReadCnt++];             // return next byte
}

// add a byte to the buffer
void putCharToTxBuf(char byte){
    if (TxBufWriteCnt>=TX_BUF_SIZE) {TxBufWriteCnt=0;}  // if buffer full reset write count to zero
    EUSART4TXbuf[TxBufWriteCnt++]=byte;                 // write byte in next spot
}
...
void TxBufferedString(char *string){
	while (*string != 0){               // while in the string
        putCharToTxBuf(*string++);      // write the next byte of the string into the buffer
    }
}
